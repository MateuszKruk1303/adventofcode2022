...
const getCommonItem = (data) => {
  const [firstCompartment, secondCompartment] = [
    ...
    data.slice(data.length / 2, data.length).split(""),
  ];
  const duplicate = firstCompartment.reduce(
    (acc, item) => (secondCompartment.includes(item) ? item : acc),
    ""
  );
  return duplicate;
};
...
const getCommonItemBetweenGroup = (data) => {
  ...
  const intersection = first
    .split("")
    .filter((item) => second.split("").includes(item))
    .filter((item) => third.split("").includes(item));
...
const evaluateItem = (item) => {
  ...
  const value =
    item.charCodeAt(0) > UPPERCASE_TOP
      ? item.charCodeAt(0) - LOWERCASE_SUBSTRACT_VALUE
      : item.charCodeAt(0) - UPPERCASE_SUBSTRACT_VALUE + UPPERCASE_OFFSET;

  return value;
};

const result = dataset.reduce(
  (acc, item) => acc + evaluateItem(getCommonItemBetweenGroup(item)),
  0
);
